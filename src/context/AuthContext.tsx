...
// 🔒 Single source of truth for the platform's Super Admin identity.
// Exact match only — no domain-wide or substring grants.
const SUPER_ADMIN_EMAIL = 'admin@onyxstacklabs.com';
...
function buildDefaultProfile(firebaseUser: FirebaseUser): UserProfile {
  ...
  return {
    ...
    displayName: firebaseUser.displayName || (superAdmin ? 'Platform Super Admin' : 'Student User'),
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
        ...
        if (firebaseUser) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              // Super Admin identity always wins over stale Firestore state,
              // but only for the exact admin account.
  ...
  const switchRole = async (newRole: UserRole) => {
    ...
    try {
      const updatedProfile = { ...profile, role: newRole, updatedAt: new Date().toISOString() };
