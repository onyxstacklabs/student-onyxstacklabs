...
function TeacherAttendance() {
  ...
  const loadClassStudents = async () => {
    if (!teacherDetails?.institutionId || !selectedClass || !subject.trim()) {
    ...
    try {
      const [allStudents, existingRecords] = await Promise.all([
        ...
        getAttendanceForClass(teacherDetails.institutionId, selectedClass, date),
      ]);
      ...
      classStudents.forEach((s) => {
        const existing = existingRecords.find((r) => r.studentUid === s.uid && r.subject === subject.trim());
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      <PageHeader
        ...
        description="Select your class and subject to record today's attendance."
      />
          ...
          {students.length === 0 ? (
            ...
            <>
              <div className="space-y-2">
                {students.map((s) => (
                  <div key={s.uid} className="flex items-center justify-between p-3.5 bg-surface-base border border-surface-border rounded-xl">
                    <div>
